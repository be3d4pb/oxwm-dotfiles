...
local tags = { "󰣙", "", "", "", "󱄅", "󱇤", "", "󰡢", "󰹡" } -- find more at https://www.nerdfonts.com/cheat-sheet
...
oxwm.set_layout_symbol("tabbed", "[Tab]")
...
oxwm.key.bind({modkey, "Shift"}, "X", oxwm.spawn({ "sh", "-c", "mkdir -p ~/Pictures/Screenshots && maim -s -u > ~/Pictures/Screenshots/$(date +%F_%H-%M).png"}))
oxwm.key.bind({modkey, "Shift"}, "C", oxwm.spawn({ "sh", "-c", 'maim -s -u | xclip -selection clipboard -t image/png'}))
oxwm.key.bind({ modkey }, "Q", oxwm.client.kill())
oxwm.key.bind({modkey}, "B", oxwm.spawn({ "librewolf" }))
oxwm.key.bind({modkey}, "E", oxwm.spawn({ "thunar" }))
oxwm.key.bind({modkey}, "P", oxwm.spawn({" pavucontrol" }))
oxwm.key.bind( {}, "AudioRaiseVolume", oxwm.spawn({"sh", "-c", "pactl set-sink-volume @DEFAULT_SINK@ +5%"}))
oxwm.key.bind( {}, "AudioLowerVolume", oxwm.spawn({"sh", "-c", "pactl set-sink-volume @DEFAULT_SINK@ -5%"}))
oxwm.key.bind( {}, "AudioMute", oxwm.spawn({"sh", "-c", "pactl set-sink-mute @DEFAULT_SINK@ toggle"}))
--oxwm.key.bind( {}, "AudioPlay", oxwm.spawm({"playerctl play-pause"}))
--oxwm.key.bind( {}, "AudioNext", oxwm.spawm({"playerctl next"}))
--oxwm.key.bind( {}, "AudioPrev", oxwm.spawm({"playerctl previous"}))
